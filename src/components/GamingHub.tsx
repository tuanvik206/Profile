        ...
        >
          ...
          <div className="flex-1 overflow-hidden min-h-0 flex flex-col">
            {activeTab === "favorite" ? (
              ...
              <div className="flex-1 min-h-0 flex flex-col gap-6 overflow-x-hidden overflow-y-auto custom-scrollbar">

                {/* ── ROW 1: Player Identity Card ── */}
                <div className="w-full">
                  <div className="border border-white/10 bg-white/[0.01] p-5 sm:p-6 relative">
                    {/* Decorative top line */}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                            ...
                            {valProfile.mainAgentUuids?.map((uuid, idx) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 border rounded-sm w-full transition-all duration-200 group text-left ${
                                    canNavigate
                      ...
                      {/* Right: Large Rank Icon and Name below */}
                      <div className="lg:border-l lg:border-white/5 lg:pl-8 flex flex-col items-center justify-center py-4 w-full h-full min-h-[180px]">
                        {valProfile.rankIcon ? (
                          <div className="flex flex-col items-center gap-3 select-none">
                            <div className="relative group">
                              {/* Outer ambient glow */}
                              <div className="absolute inset-0 bg-amber-500/10 blur-2xl rounded-full scale-90 group-hover:scale-110 transition-transform duration-700" />
                              {/* Inner ambient glow */}
                              <div className="absolute inset-0 bg-amber-500/5 blur-lg rounded-full" />
                              
                              <img
                                src={valProfile.rankIcon}
                                alt={valProfile.rankName || "Rank"}
                                className="w-28 h-28 sm:w-36 sm:h-36 object-contain relative z-10 drop-shadow-[0_0_15px_rgba(245,158,11,0.4)] group-hover:drop-shadow-[0_0_25px_rgba(245,158,11,0.6)] transition-all duration-500 scale-100 group-hover:scale-105"
                                referrerPolicy="no-referrer"
                                title={valProfile.rankName}
                              />
                            </div>
                            <span className="text-sm sm:text-base font-bold font-mono tracking-widest text-amber-500 uppercase mt-2 drop-shadow-[0_0_8px_rgba(245,158,11,0.2)]">
                              {language === 'vi' && valProfile.rankName
                                ? valProfile.rankName.toUpperCase()
                                    .replace("IRON", "SẮT")
                                    .replace("BRONZE", "ĐỒNG")
                                    .replace("SILVER", "BẠC")
                                    .replace("GOLD", "VÀNG")
                                    .replace("PLATINUM", "BẠCH KIM")
                                    .replace("DIAMOND", "KIM CƯƠNG")
                                    .replace("ASCENDANT", "TIÊN PHONG")
                                    .replace("IMMORTAL", "BẤT TỬ")
                                    .replace("RADIANT", "HÀO QUANG")
                                    .replace("UNRANKED", "CHƯA XẾP HẠNG")
                                : valProfile.rankName}
                            </span>
                          </div>
                        ) : (
                          <div className="text-[10px] font-mono text-gray-500 uppercase tracking-widest border border-dashed border-white/10 p-8 rounded-sm">
                            {t('empty_rank')}
                          </div>
                        )}
                      </div>
                ...
                <div className="w-full">
                  <div className="border border-white/10 bg-white/[0.01] p-5 sm:p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 content-start gap-4 pb-4">
                      {valProfile.favoriteSkins && valProfile.favoriteSkins.length > 0 ? (
                        valProfile.favoriteSkins.map((skin, idx) => (
                          ...
                          >
                            ...
                            <div>
                              <div className="flex justify-between items-start text-gray-500 text-[8px] font-mono">
                                <span className="px-1.5 py-0.5 border border-white/10 bg-black/40 text-amber-500 rounded-sm font-bold tracking-widest uppercase">
